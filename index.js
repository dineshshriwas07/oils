...
const PORT = process.env.PORT || 3000;
   ...
   // Razorpay Instance
    ...
    //get local serivce 
    app.get('/localService',(req , res)=>{
        res.render('localService' , { session: req.session });
    });
    ...
    app.post('/verifyPayment', (req, res) => {
        const {
            title, price, quantity, totalAmount,
        ...
        if (generated_signature === razorpay_signature) {
            ...
            (title, price, quantity, totalAmount, customerName, customerEmail, customerMobile, customerAddress, delivery_code, status, razorpay_order_id, razorpay_payment_id) 
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`;
    ...
    app.post("/cancel-order/:id", async (req, res) => {
        ...
        db.query("SELECT * FROM orders WHERE id = ?", [orderId], async (err, result) => {
            ...
            if (!order.razorpay_payment_id) {
                return res.json({ success: false, message: "Payment ID missing in DB" });
            }

            try {
                // Refund request
                const refund = await razorpay.payments.refund(order.razorpay_payment_id, {
                    amount: order.totalAmount * 100, // paise me amount
                    speed: "optimum"
                });

                // Update DB with refund ID
                db.query(
                    ...
                    (updateErr) => {
                        ...
                        return res.json({ success: true, message: "Refund successful", refund });
                    }
                );
            } catch (error) {
                // console.error("Refund Error =>", error.error || error);
                return res.json({ success: false, message: "Refund failed", error: error.error || error });
            }
        });
    });
